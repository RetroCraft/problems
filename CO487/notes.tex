...
Within Secure Connections, there are four \term*{association models}:
\begin{enumerate}[nosep]
...
\begin{scheme}[Bluetooth public key exchangge]
...
\lecture{Mar 20}
\begin{scheme}[Bluetooth authentication stage 1] 
...
\begin{scheme}[Bluetooth authentication stage 2]
...
\begin{scheme}[Bluetooth link key calculation]
...
\begin{scheme}[Bluetooth authentication and encryption]
...
\begin{defn}[random bit generator]
...
\begin{defn}[side-channel attack]
  An attack which exploits information leaked as a result of physical processes.
\end{defn}

For example, if Alice computes $aP$ with double-and-add,
an attacker can analyze how much power her computer is drawing and
correlate spikes with doubles vs. adds (which represent 0s/1s in $a$).

Alternatively, attackers have successfully leaked keys using
the high-pitched whine of a microchip, the static potential of a laptop case,
or the flickering of the power LED.

\begin{defn}[multi-party computation]
  A protocol that allows $n$ parties to jointly evaluate a function $f(x_1,\dotsc,x_n)$
  where $x_i$ is private to the $i$\xth party
  without any party revealing anything about their $x_i$.
\end{defn}

\begin{problem}[private set intersection]
  Alice has a set $X$ and Bob has a set $Y$.
  Alice wishes to determine $X \cap Y$ without revealing anything else
  (other than $\abs{X}$ and $\abs{Y}$).
\end{problem}

For example, if $X$ is a list of Alice's passwords and Bob has a list $Y$ of known compromised passwords,
Alice wants $X \cap Y$ without telling Bob $X$.

\begin{scheme}[elliptic curve PSI protocol]
  Let $E/\Z_p$ be the P-256 curve and $H : \bits{*} \to E(\Z_p)$ be a hash function.
  \begin{algorithmic}[1]
    \State Alice selects $a \randin [1,n-1]$ and sends $aH(X) \gets (aH(x_1),\dotsc,aH(x_s))$ to Bob.
    \State Bob selects $b \randin [1,n-1]$ and sends $bH(Y)$ to Alice.
    \State Alice sends $T \gets a(bH(Y)) = (u_1,\dotsc,u_t)$ to Bob.
    \State Bob sends $S \gets b(aH(X)) = (v_1,\dotsc,v_s)$ to Alice.
    \State Alice determines $X \cap Y = \{x_i : v_i \in T\}$.
    \State Bob determines $X \cap Y = \{y_j : u_j \in S\}$.
  \end{algorithmic}
  This works as long as there are no hash collisions.
\end{scheme}
