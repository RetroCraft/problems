#let Torts_Intro = [
  ...
  === Definitions
  A *tort* is a wrongful act done to the property or person of another, which is recognized by law to be as such. Its main use is to *determine and allocate compensation* for *harm that has occurred*.
  ...
  Torts are usually a result from the Courts - so the perpetual question is, *"Did the Courts get it right?"* The legislation can sometimes disagree with the courts - they can then make their own torts lol

  
  === Intentional VS Unintentional Torts
  Torts come in 2 flavours, *intentional torts* and *unintentional torts*. Intentional torts require that the action that _causes_ the tort be done intentionally, and harm to have occurred. However, the tortfeasor doesn't necessarily wish for the tort to occur.

  
  === How Torts are Made and Change
  Torts are recognized by law - this means that the Courts have judged on this before and decided that *"society's values have shifted such that this tort should be sueable"*. If a tort doesn't exist, the Courts must be convinced enough to get them to add it as a new tort!
  ...
  === Liability and Fault
  *Liability* is just as it sounds like. Who is liable for compensation related to a tort?
   #block(inset: (left: 1em))[
    ...
    ==== Fault
    Is unjustifiable injurious conduct that intentionally or carelessly disregards the interests of others. If you have fault, you are liable! This was created to help deter such faulty behaviour from happening; however it is sometimes defective in undercompensating if fault cannot be shown, and sometimes overcompensation too. Also takes more time to litigate and so on.

    ==== Vicarious Liability
    States that employers (any supervisor) are generally liable for torts their employees committed while employed. The employee is not freed from liability - both victim and employer can sue. This applies to any *reasonably foreseeable torts* committed by employees.
  ...
  The tortfeasor has a few defenses against torts (negligence or intentional or otherwise). The judge however can only pick ONE defense. This changes the decision so judge decides how harsh to be!
  #block(inset: (left: 1em))[
    ...
    The defense says Plaintiff knew *both physical and legal* risks (they'll get hurt + waived right to sue) and *voluntarily* assumed the risk. Then Plaintiff is barred from recovery (zero compensation!)
  ...
  #block(inset: (left: 1em), breakable: false)[
