...
\usegdlibrary{circular,force}
\tikzset{graphs/math nodes}
...
\begin{defn}[graph]
  A graph $G = (V, E)$ is a set of \term{vertices} $V$
  ...
  Denote the set of vertices by $V(G)$ and edges by $E(G)$.
\end{defn}
\spewnotes
...
\begin{sol}
  ...
  We can redraw $K_4$ like \tikz[baseline=0]pt\graph{ subgraph C_n [n=3, clockwise] -- 4 };
  to make it planar.
  ...
  (e.g. \tikz[baseline=0pt]\graph{ subgraph C_n [V={t_1,t_2,t_3,t_4,t_5,t_6}, clockwise, -!-] -- s_1};).

  When $m=2$, we can put $s_1$ on the left and $s_2$ on the right and connect
  (e.g. \tikz[baseline=-25pt]\graph{ s_1 -- {t_1, t_2, t_3} -- s_2 };)
...
\begin{defn}[polynomial time]
  ...
  Denote the set of such problems by $\symsf{P}$.
\end{defn}

Then, ask if the decision problem ``does $G$ contain a cycle?'' lies in $\symsf{P}$.
