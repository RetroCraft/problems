...
\begin{defn}[bijection][bijectivity]
  ...
  \term[injectivity]{injective} (i.e., $f(a) = f(a') \implies a = a'$)
  and \term[surjectivity]{surjective} (i.e., $\forall b, \exists a, f(a) = b$).
...
\begin{defn}[multiset of $t$ types][multiset]
  Sequence $a_1,\dotsc,a_t$ where each $a_i$ is a non-negative integer.
  The $a_i$ are the \term[multiset!parts]{parts}
  and the sum $\sum a_i$ is the \term[multiset!size]{size}.
\end{defn}
...
\begin{defn}[binary string]
  ...
  The \term[binary string!length]{length} of a binary string is the total number of 0's and 1's.
  We notate the \term[binary string!empty string]{empty string} by $\varepsilon = \varnothing$.
...
\begin{defn}[auxiliary (characteristic) polynomial][characteristic polynomial]
...
\begin{defn}[complex binomial coefficients][binomial coefficient!complex]
