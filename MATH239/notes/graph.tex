...
\begin{defn}[graph]
  A graph $G = (V, E)$ is a finite set of \term[vertex]{vertices} $V$
  and a set\tablefootnote{if we allow multisets, they are \term[edge!multiple]{multiple} or \term[edge!parallel]{parallel} edges}
...
\begin{defn}[complete graph on $n$ vertices][graph!complete]
...
\begin{defn}[path on $n$ vertices][path!on $n$ vertices]
  The graph $P_n$ given by $V(P_n) = [n]$ and $E(P_n) = \{ \{i,i+1\} : i \in [n-1] \}$.
\end{defn}

\begin{defn}[cycle on $n$ vertices][cycle!on $n$ vertices]
...
\begin{defn}[complete bipartite graph][graph!complete bipartite]
...
\begin{defn}
  ...
  \begin{itemize}[nosep]
    \item If $uv \in E(G)$, then $u$ and $v$ are \term[adjacency]{adjacent}
    \item If $e = uv$, then $e$ \term[joining]{joins} the \term[endpoint]{endpoints} $u$ and $v$
    \item If $v \in e$, then $e$ is \term[incidence]{incident} with $v$
...
\begin{defn*}[types of subgraphs]
  ...
  \term[subgraph!spanning]{spanning} if $V(H) = V(G)$ (only edge deletions),
  \term[subgraph!proper]{proper} if $H \neq G$,
  and \term[subgraph!induced]{induced} if $E(H) = \{uv \in E(G) : u,v \in V(H)\}$ (only vertex deletions).
\end{defn*}

\begin{defn}[walk]
  A sequence $W = (v_0,e_1,v_1,\dotsc,e_k,v_k)$ where $v_i \in V(G)$
  and $e_i = v_{i-1}v_i \in E(G)$ is a \term[walk!from $v_0$ to $v_k$]{walk from $v_0$ to $v_k$}.
  If $v_0 = v_k$, then $W$ is \term[walk!closed]{closed}.
  Call $k$ the \term[walk!length]{length} of the walk.
\end{defn}
...
\begin{defn}[cycle]
  ...
  The \term[cycle!length]{length} of a cycle $k$ is the number of edges in it
  and the \term[cycle!girth]{girth} of a graph is the length of its largest cycle.
  A \term[cycle!Hamiltonian]{Hamilton cycle} is a spanning cycle
  and a \term[graph!Hamiltonian]{Hamiltonian graph} contains one.
\end{defn}
...
\begin{defn}[polynomial time][$\P$ time]
...
\begin{defn}[nondeterministic polynomial time][$\NP$ time]
...
\begin{defn}[spanning tree][tree!spanning]
...
\begin{theorem}[5.2.3]
  ...
  then $T + e$ contains exactly one cycle $C$.\tablefootnote{This is the \term[cycle!fundamental]{fundamental cycle of $e$ with respect to $T$}}
...
\begin{prf}
  ...
  Since $e' \in \delta(X) = \delta(Y)$,\footnote{The \term[cut!fundamental]{fundamental cut of $e$ with respect to $T$}}
...
\begin{defn}[planar graph][graph!planar]
...
\begin{defn}[boundary walk][walk!boundary walk]
...
\begin{defn}[degree (length) of a face][degree!of a face]
...
\begin{defn}[edge subdivision][subdivision!edge subdivision]
...
\begin{defn}
  $M$ \term[saturation]{saturates} $v$ if $v$ is an end of an edge in $M$.

  $M$ is \term[matching!perfect]{perfect} if every vertex of $G$ is saturated by $M$.

  The \term[matching!size]{size} of $M$ is the number of edges it contains, i.e., $\abs{M}$.

  $M$ is a \term[matching!maximum]{maximum matching} if it has maximum size over all matchings of $G$.
\end{defn}
...
\begin{defn*}[alternating]
  ...
  a path $P$ in $G$ is \term[path!$M$-alternating]{$M$-alternating}
...
\begin{defn}[augmenting][path!$M$-augmenting]
...
\begin{defn}[minimum cover][cover!minimum]
